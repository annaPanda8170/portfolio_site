...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  let usedMain = [];
  function fallMain() {
    let columnNumber = Math.floor(Math.random() * 15 + 1);
    // 落ち始め最初のセル
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      ...
      // $main.css( "transform", "rotate(-60deg)");
      // $matrix.css({ display: "none" });
      // $("#background").fadeIn(6000);
      // $main.stop().animate2({ transform: "rotate(0deg)" }, 3000);
      let deg = 0;
      let nowList = 1;
      let oneDeg = 51.43;
      ...
      setTimeout(function () {
        let whichColumn;
        setInterval(function () {
          whichColumn = Math.floor(Math.random() * (usedMain.length - 1))
      ...
      $('#gmail').on('click', function () {
        ...
        // $('#gmailButton, #gmailButton::after').css("background-color", "rgb(36, 36, 36)")
        // console.log($('#gmailButton').css(backgroundColor))
      ...
      // $(".skillOpening").height($("#li7").outerHeight());
